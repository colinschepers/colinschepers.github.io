...
Ming Mang is a relatively unknown board game from Tibet and is played on a 8x8 board. It utilizes custodian capture of
...
rook in chess. It is an elimination game, as the objective is to capture all the opponent's pieces.

## Rules

* Players alternate their turns. Each player moves only one piece per turn. The player with the black pieces starts.
* A piece is moved orthogonally any number of unoccupied spaces. The moves are identical to the rook in chess.
...
+ Alternatively, if both players pass consecutively or a set time limit is reached, the player wins that has the largest
...
class MingMang is the most general class creating the game, players and GUI and runs the game loop. Class Data contains
...
or when a player has not many pieces left (note that the total amount of pieces on the board is always equal to 28).
...
During the opponent's time, CAMMP makes a guess which move the opponent is most likely going to do. This guess is done
...
according to the new position and the search done during the opponent’s play time use not used. However, if the
...
The number of pieces feature is trivial, since having more pieces than the opponent is advantageous for winning the
game. Considering two strong players a game normally ends before a player can capture all the opponent's pieces. Then
the territories are counted and the player with the most territory wins. CAMMP does not use this definition of a
territory but considers a territory to be the squares that can be reached by a player in exactly one move, which can
...
Besides the number of pieces, size of the official territory, size of territory reached in one step, number of pieces on
...
approximation of the best weights. This technique requires the AI to play against itself over and over again. Before
each match different weights are chosen and depending on the result, these weights are (partially) adopted or not.
...
collisions. The tables are emptied before each search and after a search the tables were almost completely full every
...
Alpha-beta hinges on searching the best move first, because this can then lead to pruning, speeding up the search.
...
Killer moves are moves that most likely produce a pruning on a certain depth. CAMMP uses two killer moves to increase
...
The null move technique assumes that not moving is the worst scenario for a player. It after doing a null move and
...
uses the null move technique when no null move occurred before in the search tree and when the distance to a leaf node is
...
M moves have to be searched again. CAMPP uses for C a value of 3, for M a value of 10 and the depth is again reduced 
by 2.
...
Instead of limiting the depth for non-promising moves, one could also increase the depth for promising moves. One
technique which can be used is quiescence search. Instead of returning the evaluation value what at a leaf node, this
method checks if the current position is quiet or not. If it is not quiet it continues the search until it is. For
...
could occur, possibly much more if there would be no memory constraints. Due to this, the quiescence search
...
playouts, and within the tree the node with the highest winning ratio is chosen. The only difference with QBF is
...
factor but proved to be bad moves. Unfortunately, because the opening book is fairly small and the correctness of the 
results are doubtful (due to minimal available time for the creation of the book), it is disabled by default.
...
See the table below for the average amount of nodes expanded during a depth 4 search. Expanding fewer nodes means less
...
It can be seen from the table that all techniques increase the performance, i.e. all cause the amount of nodes in the
...
pruning in the rest of the search. Another conclusion is that when only implementing simple techniques, they have a
